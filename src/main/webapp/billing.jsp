    ...
    <script>
        ...
        async function confirmApproval() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    closeApprovalModal();
                    showToast("Payment for Invoice " + currentApproval.inv + " (" + currentApproval.apt + ") approved successfully via " + currentApproval.selectedMethod + "!", "success", "Payment Approved");
                } else {
                    showToast(data.message || "Approval failed. Please try again.", "error", "Approval Failed");
                }
            } catch (err) {
                console.error(err);
                showToast("Error connecting to server. Please check your connection.", "error", "Network Error");
            } finally {
                btn.disabled = false;
        ...
        function showToast(message, type = 'success', title = '') {
            ...
            if (type === 'success') {
                ...
                defaultTitle = title || 'Payment Approved';
